...
\textit{where $\tau_{avg}$ - is the average pheromone trail value before global update execution, $f$ - is BWAS mutation factor defined in formula \ref{eq:bwas_factor} for the case of time based execution.}
...
In order to be able to perform benchmark comparison of the framework's performance, one may use publicly known instances of CVRP problems, see Appendix. To make the performance estimation more objective, one must strictly separate training (tuning) and testing set. This will remove the positive effect of some optimization decisions that may have been made during the framework development. For the training stage all Uchoa et al.(2014) instances were selected, that were of size less than 300. These instances are generated on a square grid in Euclidean space.
...
\begin{itemize}
\item Performance of a concrete selected algorithm.
\item Performance of I-Race configuration software.
\item Set-up of the configuration software, that was used to produce configuration instances.
\end{itemize}
...
[description of the obtained configuration]
