...
import solving.globalUpdate.AntColonySystem;
...
/**
 * Created by Aldar on 13-Nov-17.
 */
public class TestAntColonySystem
{
    @Test
    public void testRankBasedAntSystem()
    {
        ...
        try
        {
            ...
            GlobalUpdate update = new AntColonySystem(problem, 0.8);

            update.update(solutionList);

            assertEquals(solution2.getComponents().get(0).getPheromone(), 0.801333, 0.01);
            assertEquals(solution2.getComponents().get(1).getPheromone(), 0.801333, 0.01);
            assertEquals(solution2.getComponents().get(2).getPheromone(), 0.801333, 0.01);
            assertEquals(solution2.getComponents().get(3).getPheromone(), 0.801333, 0.01);
        }
        catch (Exception e)
        {
            e.printStackTrace();
